...
if __name__ == '__main__':
	...
	for i, m in enumerate(player.find()):
		...
		p = get_percentile(m['mmr_estimate']['estimate'])
		for i in range(1, num_heroes+1):
			hero_id = str(i)
			if p >= 0.99:
				hero_player[hero_id][player_id] = np.array(hero_perc[hero_id]['0.99'])
			elif p >= 0.95:
				w1 = (p - 0.95) / 0.04
				w2 = (0.99 - p) / 0.04
				hero_player[hero_id][player_id] = w2*np.array(hero_perc[hero_id]['0.95']) + w1*np.array(hero_perc[hero_id]['0.99'])
			elif p >= 0.9:
				w1 = (p - 0.9) / 0.05
				w2 = (0.95 - p) / 0.05
				hero_player[hero_id][player_id] = w2*np.array(hero_perc[hero_id]['0.9']) + w1*np.array(hero_perc[hero_id]['0.95'])
			elif p >= 0.8:
				w1 = (p - 0.8) / 0.1
				w2 = (0.9 - p) / 0.1
				hero_player[hero_id][player_id] = w2*np.array(hero_perc[hero_id]['0.8']) + w1*np.array(hero_perc[hero_id]['0.9'])
			elif p >= 0.7:
				w1 = (p - 0.7) / 0.1
				w2 = (0.8 - p) / 0.1
				hero_player[hero_id][player_id] = w2*np.array(hero_perc[hero_id]['0.7']) + w1*np.array(hero_perc[hero_id]['0.8'])
			elif p >= 0.6:
				w1 = (p - 0.6) / 0.1
				w2 = (0.7 - p) / 0.1
				hero_player[hero_id][player_id] = w2*np.array(hero_perc[hero_id]['0.6']) + w1*np.array(hero_perc[hero_id]['0.7'])
			elif p >= 0.5:
				w1 = (p - 0.5) / 0.1
				w2 = (0.6 - p) / 0.1
				hero_player[hero_id][player_id] = w2*np.array(hero_perc[hero_id]['0.5']) + w1*np.array(hero_perc[hero_id]['0.6'])
			elif p >= 0.4:
				w1 = (p - 0.4) / 0.1
				w2 = (0.5 - p) / 0.1
				hero_player[hero_id][player_id] = w2*np.array(hero_perc[hero_id]['0.4']) + w1*np.array(hero_perc[hero_id]['0.5'])
			elif p >= 0.3:
				w1 = (p - 0.3) / 0.1
				w2 = (0.4 - p) / 0.1
				hero_player[hero_id][player_id] = w2*np.array(hero_perc[hero_id]['0.3']) + w1*np.array(hero_perc[hero_id]['0.4'])
			elif p >= 0.2:
				w1 = (p - 0.2) / 0.1
				w2 = (0.3 - p) / 0.1
				hero_player[hero_id][player_id] = w2*np.array(hero_perc[hero_id]['0.2']) + w1*np.array(hero_perc[hero_id]['0.3'])
			elif p >= 0.1:
				w1 = (p - 0.1) / 0.1
				w2 = (0.2 - p) / 0.1
				hero_player[hero_id][player_id] = w2*np.array(hero_perc[hero_id]['0.1']) + w1*np.array(hero_perc[hero_id]['0.2'])
			else:
				hero_player[hero_id][player_id] = np.array(hero_perc[hero_id]['0.1'])
	bar.finish()

	hero_avg_attr = {}
	for hero_id in range(1, num_heroes+1): 
		hero_avg_attr[str(hero_id)] = np.zeros(7, dtype='float')

	for hero_id in hero_player.keys():
		for player_id in hero_player[hero_id].keys():
			if float('Nan') in hero_player[hero_id][player_id]:
				continue
			hero_avg_attr[hero_id] += hero_player[hero_id][player_id]

	for hero_id in range(1, num_heroes+1): 
		hero_avg_attr[str(hero_id)] /= N
		hero_player[str(hero_id)]['unknown_player'] = hero_avg_attr[str(hero_id)]
